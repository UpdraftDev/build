...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^updraft_") ; then
       UPDRAFT_BUILD=$(echo -n $1 | sed -e 's/^updraft_//g')
    else
       UPDRAFT_BUILD=
    fi
    export UPDRAFT_BUILD
...
function breakfast()
{
    ...
    for f in `/bin/ls vendor/updraft/vendorsetup.sh 2> /dev/null`
        do
echo "including $f"
            . $f
        done
...
echo "z$target" | grep -q "-"
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the updraft model name
            lunch updraft_$target-userdebug
        fi
fi
return $?
}
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the Updraft github
        T=$(gettop)
        pushd $T > /dev/null
